		...
		<table class="table table-striped">
			...
			<tbody>
				<c:if test="${not empty bList }">
					<c:forEach items="${bList }" var="bookparcel" varStatus="status">
						<tr>
							...
							<td style="width: 7%;"><a
								href="mypageInfo.do?userId=${bookparcel.userId }">${bookparcel.userNo }</a>
							</td>
							<td>${bookparcel.addr }</td>
							<td style="width: 7%;"><a
								href="parcelSuccess.do?deliveryNo=${bookparcel.deliveryNo}"
								onclick="return confirm('완료처리 하시겠습니까?');">완료</a></td>
						</tr>
					</c:forEach>
				</c:if>
				...
				<tr align="center" height="20">
					...
					<td colspan="8"><c:url var="before" value="bookParcelList.do">
					...
					[이전]
						...
						</c:forEach> <c:url var="before" value="bookParcelList.do">
							<c:param name="page" value="${pi.currentPage + 1 }"></c:param>
